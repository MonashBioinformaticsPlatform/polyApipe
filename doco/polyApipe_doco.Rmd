...
3. **Load into R. ** The polyApiper R package will prepare a 'banquet' of results for alternative polyA (APA) analysis ...
4. **APA Analysis** ...
...
PolyApipe comes in two parts; a python3 script polyApipe.py and the R packages polyApiper.


The PolyApipe.py python script depends on the following tools:

 * [samtools](http://www.htslib.org/) [@Li2009]
 * [UMI-tools](https://github.com/CGATOxford/UMI-tools) [@Smith2017]
 * [featureCounts](http://subread.sourceforge.net/) (version 1.5.3 or above) from Subread package [@Liao2014]
 * [pysam](https://github.com/pysam-developers/pysam) python module

The quickest way to get that all installed (if you don't want to install everythign seperately) is to use [conda](https://docs.conda.io/en/latest/). 
...
Install the python (python3) part:

```{sh eval=FALSE}
# globally
pip install git+https://github.com/swbioinf/polyApipe
# Or in home directory, adding to path if neccessary
...
Finally, to install the polyAiper R package - run the following commands within R:
...
<!-- Todo: put polyApipe into its own conda? Yuck. -->
...
organism <- load_banquet("human_ens94")
